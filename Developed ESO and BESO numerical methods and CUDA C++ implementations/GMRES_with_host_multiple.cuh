void FFT_GMRES_with_CUDA_extended_with_host_memory_multiple(const cuComplex *dev_gamma_array, const bool *dev_mask, cuComplex *dev_solution, float **dev_actual_residual, unsigned int *GMRES_n, cufftHandle plan, cublasHandle_t *handle_p, const float tolerance, const bool for_gradient, const unsigned int h_index_of_max)
{
	...
	cuComplex *dev_orthogonal_set;
	dev_orthogonal_set = NULL;
...
//=============================================== End: Condition to iterate ===========================================================
	...
	//======================================================= END: Update residual ======================================================
		...
		cudacall(cudaMalloc((void**)&dev_orthogonal_set, Batch_size * N * N * sizeof(cuComplex)));

		for(GMRES_i = 1; ((*res_vs_tol) && (GMRES_i < 1000)); GMRES_i ++)
		{
			...
			for(unsigned int j = 0; j < GMRES_i + 1; j++)
			{
				if (j % Batch_size == 0)
				{
					if (GMRES_i > j + Batch_size)
					{
						cudacall(cudaMemcpy(dev_orthogonal_set, h_orthogonal_basis + j * N * N, Batch_size * N * N * sizeof(cuComplex), cudaMemcpyHostToDevice));
					}
					else
					{
						cudacall(cudaMemcpy(dev_orthogonal_set, h_orthogonal_basis + j * N * N, (GMRES_i - j) * N * N * sizeof(cuComplex), cudaMemcpyHostToDevice));
					}
				}
				get_new_w_and_H((cuComplex **)&dev_w, (cuComplex *)(dev_H_ + j * (GMRES_i + 1) + GMRES_i), (cuComplex *)dev_orthogonal_set + j % Batch_size);//Fill Hessenberg matrix
			}
		...
		//============================================== BEGIN: Fill Orthogonal Basis m.============================================
			...
			fprintf(stderr, "D\n");
			cudacall(cudaMemcpy(h_orthogonal_basis + GMRES_i * N * N, dev_orthogonal_vec, N * N * sizeof(cuComplex), cudaMemcpyDeviceToHost));
	...
	cudacall(cudaFree((cuComplex *)dev_orthogonal_set));
